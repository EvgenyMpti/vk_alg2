...
class TreeOptTest : public ::testing::Test {
};

TEST_F(TreeOptTest, BuildTreeOptBasic) {
    std::vector<std::optional<int>> arr = { 8, 9, 11, 7, 16, 3, 1 };
    TreeNode* root = buildTreeOpt(arr);
...
TEST_F(TreeOptTest, BuildTreeOptWithNulls) {
...
TEST_F(TreeOptTest, IsSymmetricOpt) {
    // Симметричное
    std::vector<std::optional<int>> sym_arr = { 3, 8, 8, 9, 6, 6, 9 };
    TreeNode* sym_root = buildTreeOpt(sym_arr);
    EXPECT_TRUE(isSymmetricOpt(sym_root));
    delete sym_root;

    // Асимметричное
    std::vector<std::optional<int>> asym_arr = { 3, 8, 7, 9, 6, std::nullopt, 9 };
    TreeNode* asym_root = buildTreeOpt(asym_arr);
    EXPECT_FALSE(isSymmetricOpt(asym_root));
    delete asym_root;

    // + асимметричное
    std::vector<std::optional<int>> asym_arr2 = { 1, 2, 2, std::nullopt, 3, std::nullopt, 3 };
    TreeNode* asym_root2 = buildTreeOpt(asym_arr2);
    EXPECT_FALSE(isSymmetricOpt(asym_root2));
    delete asym_root2;

    EXPECT_TRUE(isSymmetricOpt(nullptr));
}

TEST_F(TreeOptTest, MinDepthOpt) {
    ...
    EXPECT_EQ(minDepthOpt(depth_root), 3);
    delete depth_root;

    TreeNode* simple_root = new TreeNode(1);
    simple_root->left = new TreeNode(2); // Лист
    EXPECT_EQ(minDepthOpt(simple_root), 2);
    delete simple_root;

    EXPECT_EQ(minDepthOpt(nullptr), 0);
}

TEST_F(TreeOptTest, MaxMinProductBST) {
    ...
    EXPECT_EQ(maxMinProductBST(bst_root), 14);
    delete bst_root;

    TreeNode* single_node_bst = new TreeNode(5);
    EXPECT_EQ(maxMinProductBST(single_node_bst), 25);
    delete single_node_bst;

    EXPECT_THROW(maxMinProductBST(nullptr), std::invalid_argument);
}

TEST_F(TreeOptTest, IsSameTreeUnchanged) {
    std::vector<std::optional<int>> arr1 = { 1, 2, 3 };
    std::vector<std::optional<int>> arr2 = { 1, 2, 3 };
    std::vector<std::optional<int>> arr3 = { 1, 2, 4 };
    TreeNode* t1 = buildTreeOpt(arr1);
    TreeNode* t2 = buildTreeOpt(arr2);
    TreeNode* t3 = buildTreeOpt(arr3);
...
TEST_F(TreeOptTest, IsSubtreeUnchanged) {
    ...
    TreeNode* rootA = buildTreeOpt(arrA);
    TreeNode* rootC = buildTreeOpt(arrC);
...
TEST_F(TreeOptTest, CountMirrorNodesUnchanged) {
    std::vector<std::optional<int>> mirror_arr = { 3, 8, 8, 9, 6, 6, 9 };
    TreeNode* mirror_root = buildTreeOpt(mirror_arr);
    // (8,8), (9,9), (6,6) -> 3
